...
export const loanTokenBal = async () => {
    ...
    let loanTokenNameSet: Set<number | string> = new Set();

    const tusdOutFilter = { address: contracts.tusd, topics: tusd.filters.Transfer(contracts.tfi).topics, fromBlock: 11280398, toBlock: "latest" }
    const tusdInFilter = { address: contracts.tusd, topics: tusd.filters.Transfer(null, contracts.tfi).topics, fromBlock: 11280398, toBlock: "latest" }

    const curveOutFilter = { address: contracts.curveGauge, topics: curveGauge.filters.Withdraw(contracts.tfi).topics, fromBlock: 11280398, toBlock: "latest" }
    const curveInFilter = { address: contracts.curveGauge, topics: curveGauge.filters.Deposit(contracts.tfi).topics, fromBlock: 11280398, toBlock: "latest" }
    ...
    let usdArray = processArray(tusdArray, "TUSD");
    let curvArray = processArray(curveArray, "yCRV");

    totalLoanHistoricalBal = totalLoanHistoricalBal.concat(usdArray);
    totalLoanHistoricalBal = totalLoanHistoricalBal.concat(curvArray);
